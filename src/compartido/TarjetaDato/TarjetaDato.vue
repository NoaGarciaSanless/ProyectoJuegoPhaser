...
onBeforeUnmount(() => {
    ...
    if (app) {
        if (props.dato.spriteURL && PIXI.Assets.cache.has(props.dato.spriteURL)) {
            PIXI.Assets.unload(props.dato.spriteURL);
        }
...
<template>
    <div class="tarjeta" ref="tarjetaRef">
        ...
        <div class="datos">
            ...
            <button v-if="props.dato.jsonURL" class=" verBTN" @click="$emit('mostrarDetalles', dato)">
                Ver detalles
            </button>
        </div>
    </div>
